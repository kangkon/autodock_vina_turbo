...
void do_randomization(model& m,
	...
	m.set(best_conf);
	if(verbosity > 1) {
		log << "[INFO] Clash penalty: " << best_clash_penalty; // FIXME rm?
		log.endl();
	}
...
void do_search(model& m, const boost::optional<model>& ref, const scoring_function& sf, const precalculate& prec, const igrid& ig, const precalculate& prec_widened, const igrid& ig_widened, non_cache& nc, // nc.slope is changed
	...
	if(score_only) {
		...
		log << "[INFO] Affinity: " << std::fixed << std::setprecision(5) << e << " (kcal/mol)";
		log.endl();
		flv term_values = t.evale_robust(m);
		VINA_CHECK(term_values.size() == 5);
		log << "[RESULT]\n";
		log << "[REMARK] Method: score_only\n";
		log << "[REMARK] Intermolecular contributions to the terms, before weighting:\n";
		...
		if(e < 100 && std::abs(e2 - e) > 0.05) {
			log << "[WARNING] the individual terms are inconsisent with the affinity. Consider reporting this as a bug: http://vina.scripps.edu/manual.html#bugs\n";
		}
	}
	else if(local_only) {
		output_type out(c, e);
		...
		log << "[RESULT]\n";
		log << "[REMARK] Method: local_only\n";
		log << "Affinity (kcal/mol): " << std::fixed << std::setprecision(5) << e << '\n';
		log << "[ENDRESULT]\n";
		log.endl();
		if(!nc.within(m))
			log << "[WARNING] not all movable atoms are within the search space\n";

		doing(verbosity, "[INFO] Writing output", log);
		...
		log.endl();
		output_container out_cont;
		doing(verbosity, "[INFO] Performing search", log);
		par(m, out_cont, prec, ig, prec_widened, ig_widened, corner1, corner2, generator);
		done(verbosity, log);

		doing(verbosity, "[INFO] Refining results", log);
		...
		log << "[RESULT]\n";
		log << "[REMARK] Method: default\n";
		log << "[REMARK] Result Table - mode, affinity (kcal/mol) and RMSD of distance from best mode\n";
		log << "mode | affinity (kcal/mol) | rmsd l.b.| rmsd u.b.\n";
		log << "-----|---------------------|----------|----------\n";
		...
		VINA_FOR_IN(i, out_cont) {
			...
			++how_many;
			log << std::setw(4) << i+1
				<< " | " << std::setw(19) << std::setprecision(1) << out_cont[i].e; // intermolecular_energies[i];
			m.set(out_cont[i].c);
			const model& r = ref ? ref.get() : best_mode_model;
			const fl lb = m.rmsd_lower_bound(r);
			const fl ub = m.rmsd_upper_bound(r);
			log << " | " << std::setw(8) << std::setprecision(3) << lb
...
void main_procedure(model& m, const boost::optional<model>& ref, // m is non-const (FIXME?)
	...
	doing(verbosity, "[INFO] Setting up the scoring function", log);
	...
	if(randomize_only) {
		...
		if(no_cache) {
			...
			if(cache_needed) doing(verbosity, "[INFO] Analyzing the binding site", log);
...
void check_occurrence(boost::program_options::variables_map& vm, boost::program_options::options_description& d) {
	VINA_FOR_IN(i, d.options()) {
		...
		if(!vm.count(str))
			std::cerr << "[ERROR] Required parameter --" << str << " is missing!\n";
	}
}
...
int main(int argc, char* argv[]) {
	...
	const std::string version_string = "[INFO] AutoDock Vina 1.1.2 (May 11, 2011)";
	const std::string error_message = "[ERROR] Please contact the author for a fix or report a bug at our github page";

	const std::string cite_message = "[INFO] AutoDock Vina 1.1.2 - Edited by Kangkon Saikia\n";
	...
	try {
		...
		if(output_produced) { // FIXME
			if(!vm.count("out")) {
				...
				log << "[INFO] Output will be " << out_name << '\n';
			}
		}
		...
		if(vm.count("cpu") == 0) {
			...
			if(verbosity > 1) {
				if(num_cpus > 0)
					log << "[INFO] Detected " << num_cpus << " CPU" << ((num_cpus > 1) ? "s" : "") << '\n';
				else
					log << "[WARNING] Could not detect the number of CPUs, using 1\n";
			}
			if(num_cpus > 0)
				cpu = num_cpus;
			else
		...
		doing(verbosity, "[INFO] Reading input", log);
		...
		std::cerr << "\n\n[Error] File system error: " << e.what() << '\n';
		return 1;
	}
	catch(usage_error& e) {
		std::cerr << "\n\n[Error] Usage error: " << e.what() << ".\n";
		return 1;
	}
	catch(parse_error& e) {
		std::cerr << "\n\n[Error] Parse error on line " << e.line << " in file \"" << e.file.string() << "\": " << e.reason << '\n';
		return 1;
	}
	catch(std::bad_alloc&) {
	...
	// Errors that shouldn't happen:
		...
		std::cerr << "\n\n[Error] An error occurred: " << e.what() << ". " << error_message;
		return 1; 
	}
	catch(internal_error& e) {
		std::cerr << "\n\n[Error] An internal error occurred in " << e.file << "(" << e.line << "). " << error_message;
		return 1;
	}
	catch(...) {
		std::cerr << "\n\n[Error] An unknown error occurred. " << error_message;
		return 1;
	}
}
